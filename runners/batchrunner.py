...
mode = 'normal'
# mode = 'batchrunner'
# mode = 'parameter_sweep' # only sync model
...
range_parameter_sweep = 1
parameter_sweep_dict = {'max_horizon': [horizon_low, horizon_high, range_parameter_sweep],
                        'battery_capacity': [batt_low, batt_high, range_parameter_sweep]}
...
if mode == 'batchrunner' or mode == 'normal':
    if model == 'sync':
...
if mode == 'parameter_sweep':
    if model == 'sync':
        folder_paramsweep = 'Param_sweep_Sync'
...
def run_mg(sim_steps, N, model_run_mg, args_run_mg):
    ...
    """ source https://www.eia.gov/tools/faqs/faq.php?id=97&t=3, around 30 kWh a day"""
    USA_daily_household_load = 30
    USA_simulation_household_load = USA_daily_household_load * days
    factor_daily_USA = avg_consumption_day/USA_daily_household_load
    print(factor_daily_USA)

    print('average production of 1 prosumer daily:', avg_production_day)
    print('average consumption of 1 consumer daily:', avg_consumption_day)
    ...
    for agent in range(N):
        for step in range(sim_steps):
            if N <= N_max:
                big_data_file[step][agent][0] = load_file_agents_time[agent][step]**0.5 * sine_wave_consumption_series[step]*agent_char_load[agent] / factor_daily_USA
                big_data_file[step][agent][1] = production_file_agents_time[agent][step] * agent_char_prod[agent] * factor * 0.95 / factor_daily_USA
            if N > N_max:
                # if community_size == 'large':
    ...
    if model == 'pso':
        ...
        if plots == 'on':
            ...
            close_all()
            plot_iterations(num_global_iteration_over_time, num_buyer_iteration_over_time,
                            num_seller_iteration_over_time, sim_steps)
            plot_costs_over_time(E_demand_list_over_time, E_allocated_list_over_time, payment_list_over_time,
                                 E_total_supply_list_over_time, E_actual_supplied_list_over_time,
                                 revenue_list_over_time, profit_list_over_time, profit_list_summed_over_time, N,
                                 sim_steps)
            plot_control_values(w_sharing_factors_list_over_time, E_actual_supplied_list_over_time,
                                E_demand_list_over_time, c_prices_over_time, number_of_buyers_over_time,
                                number_of_sellers_over_time, sim_steps, N)
            plot_supplied_vs_surplus_total(actual_batteries_over_time, surplus_on_step_over_time,
                                           E_total_supply_over_time, E_actual_supplied_list_over_time,
                                           E_demand_over_time, N, sim_steps)
            plot_avg_soc_preferred(actual_batteries_over_time, socs_preferred_over_time, avg_soc_preferred_over_time,
                                   actual_batteries_over_time, deficit_total_over_time,
                                   deficit_total_progress_over_time, production_series_total, N, sim_steps,
                                   number_prosumers)
...
def run_mg_praram_sweep(sim_steps, N, model_run_mg, args_run_mg, sweep_num):
    ...
    """Model creation"""
    if model == 'sync':
        model_run = MicroGrid_sync_param_sweep(big_data_file, starting_point, N, lambda_set_run_mg, parameter_sweep_dict_run_mg, sweep_num)
    ...
    if model == 'pso':
        ...
        if mode == 'parameter_sweep':
            np.savez('/Users/dirkvandenbiggelaar/Desktop/result_files/' + folder_paramsweep + '/' + str(model) + '_sweep' + str(sweep) + '.npz',
                     profit_list_summed_over_time, number_of_buyers_over_time, number_of_sellers_over_time,
                     supplied_over_time_list, E_demand_over_time,
                     c_nominal_over_time, deficit_total_over_time, deficit_total_progress_over_time,
                     avg_soc_preferred_over_time, surplus_on_step_over_time, w_nominal_over_time,
        ...
        if plots == 'on':
            ...
            plot_iterations(num_global_iteration_over_time, num_buyer_iteration_over_time, num_seller_iteration_over_time,
                            sim_steps)
            plot_costs_over_time(E_demand_list_over_time, E_allocated_list_over_time, payment_list_over_time,
                                 E_total_supply_list_over_time, E_actual_supplied_list_over_time, revenue_list_over_time,
                                 profit_list_over_time, profit_list_summed_over_time, N, sim_steps)
            plot_control_values(w_sharing_factors_list_over_time, E_actual_supplied_list_over_time, E_demand_list_over_time,
                                c_prices_over_time, number_of_buyers_over_time, number_of_sellers_over_time, sim_steps, N)
            plot_supplied_vs_surplus_total(actual_batteries_over_time, surplus_on_step_over_time, E_total_supply_over_time,
                                           E_actual_supplied_list_over_time, E_demand_over_time, N, sim_steps)
            plot_avg_soc_preferred(actual_batteries_over_time, socs_preferred_over_time, avg_soc_preferred_over_time,
                                   actual_batteries_over_time, deficit_total_over_time, deficit_total_progress_over_time,
                                   production_series_total, N, sim_steps, number_prosumers)
    ...
    for num_agents in range(agents_low, agents_high):
        ...
        if num_agents % every_ == 0:
            ...
            if model == 'sync':
                ...
                exit('batchunner not available for this model')
...
if mode == 'parameter_sweep':
    from source.MicroGrid_sync_param_sweep import *
    N = normal_batchrunner_N
    for sweep in range(range_parameter_sweep):
        if model == 'sync':
            tic()
            radius = None
            global_mean, buyer_mean, seller_mean, length_sim = run_mg_praram_sweep(sim_steps, normal_batchrunner_N, model, args, sweep)
            Elapsed_time = toc()
        else:
            exit('parameter-sweep only for sync model for now')
        Elapsed_time_sweep = np.append(Elapsed_time_batch, Elapsed_time)

    np.save('/Users/dirkvandenbiggelaar/Desktop/result_files/'+ folder_paramsweep + '/list_mean_iterations_sweep', list_mean_iterations_batch)
    np.save('/Users/dirkvandenbiggelaar/Desktop/result_files/'+ folder_paramsweep + '/sweep_id', [model, batt_low, batt_high, range_parameter_sweep, length_sim, N])
    np.save('/Users/dirkvandenbiggelaar/Desktop/result_files/'+ folder_paramsweep + '/Elapsed_time_sweep', Elapsed_time_batch)
